...
class Api {
	 ...
	 * @param {String} apiKey 私服apiKey
	 * @param {String} beatmapApiKey 谱面apikey（一般为osu官服api）
	 * @param {Object} [options]
	 * @param {String} [options.baseUrl="https://osu.ppy.sh/api"] 私服 api url
	 * @param {String} [options.beatmapBaseUrl="https://osu.ppy.sh/api"] 谱面 api url（一般为osu官服api）
	 ...
	 */
	constructor(apiKey, beatmapApiKey, options = { }) {
		this.apiKey = apiKey || "";
		this.beatmapApiKey = beatmapApiKey || "";
		this.baseUrl = options.baseUrl || 'https://osu.ppy.sh/api';
		this.beatmapBaseUrl = options.beatmapBaseUrl || 'https://osu.ppy.sh/api';
	 ...
	 */
	async apiCall(endpoint, options, isBeatmap = false) {
		let url;
		if (isBeatmap){
			url = this.beatmapBaseUrl;
			if (this.beatmapApiKey) options.k = this.beatmapApiKey;
		} else {
			url = this.baseUrl;
			if (this.apiKey) options.k = this.apiKey;
		}

		try {
			const resp = await request.get(url + endpoint)
	...
	async getBeatmaps(options) {
		const resp = await this.apiCall('/get_beatmaps', options, true);
